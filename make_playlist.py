#!/usr/bin/python3
"""Build M3U8 IPTV playlists from markdown channel lists and EPG sources."""

import os
import re

EPG_LIST = open('epglist.txt', 'r', encoding='utf-8')  # for a clean code

class Channel:
    """Represents an IPTV channel parsed from a markdown table row."""
    ...
    def to_m3u_line(self):
        """Return the channel as a #EXTINF line plus URL for M3U/M3U8 format."""
        if self.epg is None:
            return (
                f'#EXTINF:-1 tvg-name="{self.name}" tvg-logo="{self.logo}" '
                f'group-title="{self.group}",{self.name}\n{self.url}'
            )
        else:
            return (
                f'#EXTINF:-1 tvg-name="{self.name}" tvg-logo="{self.logo}" '
                f'tvg-id="{self.epg}" group-title="{self.group}",{self.name}\n{self.url}'
            )

def main():
    """Generate M3U8 playlists from markdown channel lists in the lists directory."""
    ...
    with open("playlist.m3u8", "w", encoding='utf-8') as playlist:
        ...
        for filename in sorted(os.listdir(".")):
            ...
            with open(filename, encoding='utf-8') as markup_file:
                file_country = os.path.join(
                    "..", dir_playlists, "playlist_" + filename[:-3:] + ".m3u8"
                )
